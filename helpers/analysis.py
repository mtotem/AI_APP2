...
class Extent_5d:
    ...
    Accesseurs:
        ...
        get_corners: retourne les coordonnées des points aux coins d'un range couvert par les min max
    """
    def __init__(self, xmin=0, xmax=10, ymin=0, ymax=10, zmin=0, zmax=10, wmin=0, wmax=10, vmin=0, vmax=10, ptList=None):
        """
        Constructeur
        2 options:
            passer 4 arguments min et max
        ...
        if ptList is not None:
            ...
            self.vmin = np.floor(np.min(ptList[:, 4])) - 1
            self.vmax = np.ceil(np.max(ptList[:, 4])) + 1
        else:
            self.xmin = xmin
            self.xmax = xmax
            self.ymin = ymin
    ...
    def get_array(self):
        ...
        return [[self.xmin, self.xmax], [self.ymin, self.ymax], [self.zmin, self.zmax],
                [self.wmin, self.wmax], [self.vmin, self.vmax]]
